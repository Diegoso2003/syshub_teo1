...
import { blogsPaginadosPerfil, totalBlogsPub } from '../../services/MiPerfilService'
...
async function cargarPosts(pagina: number) {
    try {
        const response = await blogsPaginadosPerfil(pagina)
...
async function cargarTotales() {
    try {
        const response = await totalBlogsPub();
