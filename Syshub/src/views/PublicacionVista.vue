...
import { eliminarVoto, votar } from '../services/VotoService';
...
function irADetalle() {
  router.push(`/inicio/publicacion/${props.post.id}`)
}

async function darLike() {
  try {
    if (props.post.dio_like) {
      await eliminarVoto(props.post.id)
      props.post.dio_like = false
      props.post.likes--
    } else {
      await votar(props.post.id, true);
      props.post.dio_like = true;
      props.post.likes++
      if(props.post.dio_dislike) props.post.dislikes--
      props.post.dio_dislike = false;
    }
  } catch (error: any) {
...
async function darDislike() {
  try {
    if (props.post.dio_dislike) {
      await eliminarVoto(props.post.id)
      props.post.dio_dislike = false;
      props.post.dislikes--
    } else {
      await votar(props.post.id, false);
      if(props.post.dio_like) props.post.likes--
      props.post.dio_like = false;
      props.post.dislikes++
      props.post.dio_dislike = true;
    }
  } catch (error: any) {
    const mensaje: string = error.message || "error al iniciar sesión intente más tarde"
    alert(mensaje)
